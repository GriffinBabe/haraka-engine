...
{
    mutex.lock();
    std::cout << "[" << std::this_thread::get_id() << "] Begin of worker thread"
              << std::endl;
    mutex.unlock();
    ...
    while (true) {
        try {
            service->run(ec);
            if (ec) { // this is a system exception, so we break out of the loop
                      // to finish the thread from the thread pool.
                mutex.lock();
                std::cout << "[" << std::this_thread::get_id()
                          << "] Error code returned: " << ec << std::endl;
                mutex.unlock();
            }
            break;
        }
        catch (std::exception const&
                   exc) { // this is a context exception, so we keep our thread
    ...
    mutex.lock();
    std::cout << "[" << std::this_thread::get_id() << "] End of worker thread"
              << std::endl;
    mutex.unlock();
}
...
void calculate_fib(size_t n)
{
    mutex.lock();
    std::cout << "[" << std::this_thread::get_id()
              << "] Starting calculating fibonacci for n = " << n << std::endl;
    mutex.unlock();

    size_t val = fib(n);

    mutex.lock();
    std::cout << "[" << std::this_thread::get_id() << "] Fibonacci result for "
              << n << " is " << val << std::endl;
    mutex.unlock();
}
                   ...
                   std::shared_ptr<boost::asio::io_service::strand> strand)
{
    if (error) {
        ...
        std::cout << "[" << std::this_thread::get_id() << "] Time Handler "
                  << std::endl;
        mutex.unlock();
        ...
        timer->async_wait(
            strand->wrap(boost::bind(&timer_handler, _1, timer, strand)));
    }
}
...
int main(int argc, char** argv)
{
    ...
    // strand makes us sure that the given post and dispatch orders are done
    // sequentially
    auto strand = std::make_shared<boost::asio::io_service::strand>(*service);

    mutex.lock();
    std::cout << "[" << std::this_thread::get_id() << "] Starting the program."
              << std::endl;
    mutex.unlock();
    ...
    for (int i = 0; i < threads; i++) {
        thread_list.emplace_back(
            std::thread(boost::bind(&worker_thread, service)));
    }
    ...
    // by destroying the work object, all remaining tasks will be executed by
    // the service, then the service will stop
